...
/// ZeroFS extension for catalog-authenticated permanent checkpoints.
pub(crate) async fn create_clone_with_shared_source_pins<
    P: Into<Path>,
    R: RangeBounds<Bytes> + Clone,
>(
    clone_sources: Vec<CloneSourceSpec<R>>,
    clone_path: P,
    object_stores: ObjectStores,
...
pub(crate) async fn create_clone_with_shared_source_pins_and_checkpoint<
    P: Into<Path>,
    R: RangeBounds<Bytes> + Clone,
>(
    clone_sources: Vec<CloneSourceSpec<R>>,
    ...
    create_clone_inner(
        ...
        segment_projection,
        true,
        Some((checkpoint_id, checkpoint_name)),
    )
    .await
}

async fn create_clone_inner<P: Into<Path>, R: RangeBounds<Bytes> + Clone>(
    clone_sources: Vec<CloneSourceSpec<R>>,
    clone_path: P,
    object_stores: ObjectStores,
    fp_registry: Arc<FailPointRegistry>,
    ...
    initial_checkpoint: Option<(Uuid, String)>,
) -> Result<StoredManifest, SlateDBError> {
    let clone_path = clone_path.into();

    validate_clone_source_specs(clone_sources.clone(), clone_path.clone())?;
    if shared_source_pins && clone_sources.len() != 1 {
        return Err(SlateDBError::InvalidUnion(
            "shared source pins require exactly one clone source".to_string(),
        ));
    }
...
async fn create_clone_manifest<R: RangeBounds<Bytes> + Clone>(
    ...
    let clone_manifest =
        match StoredManifest::try_load(clone_manifest_store.clone(), system_clock.clone()).await? {
            ...
            None => {
                ...
                if let Some((checkpoint_id, checkpoint_name)) = &initial_checkpoint {
                    manifest.core.initialized = true;
                    manifest.core.checkpoints.push(Checkpoint {
                        id: *checkpoint_id,
                        manifest_id: 1,
                        expire_time: None,
                        create_time: system_clock.now(),
    ...
    for external_db in &clone_manifest.manifest().external_dbs {
        ...
        let preloaded_shared_source = shared_source_pins
            && source_specs.iter().any(|source| {
                source.path.to_string() == external_db.path
                    && source.preloaded_manifest.is_some()
                    && source.checkpoint == Some(final_checkpoint_id)
            });
        if preloaded_shared_source {
            continue;
        }
...
async fn build_sources<R: RangeBounds<Bytes> + Clone>(
    ...
    for source in source_specs {
        let (checkpoint, mut manifest_at_checkpoint) = if let Some(preloaded) =
            &source.preloaded_manifest
        {
            let checkpoint_id = source.checkpoint.ok_or(SlateDBError::InvalidDBState)?;
            let checkpoint = source
                .preloaded_checkpoint
                .as_ref()
                .ok_or(SlateDBError::InvalidDBState)?;
            if checkpoint.id != checkpoint_id
                || checkpoint.manifest_id != preloaded.id()
                || !preloaded.initialized()
            {
                return Err(SlateDBError::InvalidDBState);
            }
            (checkpoint.clone(), preloaded.manifest.clone())
        } else {
            ...
            let checkpoint = get_or_create_parent_checkpoint(
                &mut latest_manifest,
                source.checkpoint,
                rand.clone(),
            )
