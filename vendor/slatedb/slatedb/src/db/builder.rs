...
pub struct CloneSourceSpec<R: RangeBounds<Bytes> + Clone = (Bound<Bytes>, Bound<Bytes>)> {
    ...
    /// An immutable source manifest already authenticated by the caller.
    ///
    /// This is used by storage authorities that separately verify the exact
    /// checkpoint and retain its physical pin. Ordinary callers should leave
    /// it unset so clone creation discovers and validates the source itself.
    pub(crate) preloaded_manifest: Option<crate::manifest::VersionedManifest>,
    pub(crate) preloaded_checkpoint: Option<crate::checkpoint::Checkpoint>,
}
...
impl<R: RangeBounds<Bytes> + Clone> Clone for CloneSourceSpec<R> {
    fn clone(&self) -> Self {
        CloneSourceSpec {
            ...
            preloaded_manifest: self.preloaded_manifest.clone(),
            preloaded_checkpoint: self.preloaded_checkpoint.clone(),
        }
    }
}
...
impl<R: RangeBounds<Bytes> + Clone> CloneSourceSpec<R> {
    ...
    /// Reuse an exact source manifest authenticated by the storage authority.
    ///
    /// The clone still writes conditionally and callers remain responsible for
    /// verifying the resulting root and its retained source pin before making
    /// it visible.
    pub fn with_preloaded_manifest(
        mut self,
        checkpoint: crate::checkpoint::Checkpoint,
        manifest: crate::manifest::VersionedManifest,
    ) -> Self {
        self.preloaded_checkpoint = Some(checkpoint);
        self.preloaded_manifest = Some(manifest);
        self
    }
}
...
pub struct CloneBuilder<R: RangeBounds<Bytes> + Clone = (Bound<Bytes>, Bound<Bytes>)> {
    ...
    shared_source_pins: bool,
}

/// Durable result of creating a clone and its operation-stable root checkpoint.
pub struct CloneCheckpointResult {
    pub checkpoint: crate::checkpoint::Checkpoint,
    pub manifest: crate::manifest::VersionedManifest,
}
...
impl<R: RangeBounds<Bytes> + Clone> CloneBuilder<R> {
    ...
    /// Reuse permanent source checkpoints as clone pins instead of allocating
    /// one private checkpoint per clone.
    ///
    /// ZeroFS extension: callers must keep every source checkpoint permanent
    /// for the lifetime of all descendants. Shared pins are encoded by
    /// `final_checkpoint_id == source_checkpoint_id`; detach GC removes the
    /// exhausted external entry without deleting that borrowed checkpoint.
    /// This mode is deliberately restricted to a single source.
    pub fn with_shared_source_pins(mut self) -> Self {
        self.shared_source_pins = true;
        self
    }
    ...
    pub async fn build(self) -> Result<(), crate::Error> {
        ...
        let object_stores = ObjectStores::new(self.object_store, self.wal_object_store);
        let fp_registry = Arc::new(FailPointRegistry::new());
        let system_clock = self
            .system_clock
            .unwrap_or_else(|| Arc::new(DefaultSystemClock::new()));
        let rand = self.rand.unwrap_or_else(|| Arc::new(Default::default()));
        let _ = if self.shared_source_pins {
            crate::clone::create_clone_with_shared_source_pins(
                sources,
                clone_path,
                object_stores,
                fp_registry,
    ...
    /// Build the clone and durably install one permanent root checkpoint with
    /// a caller-stable identity, returning the exact authenticated manifest
    /// without reopening the destination through a second Admin client.
    pub async fn build_with_stable_checkpoint(
        self,
        checkpoint_id: Uuid,
        checkpoint_name: &str,
    ) -> Result<CloneCheckpointResult, crate::Error> {
        let sources = self.sources;
        let clone_path = self.clone_path;
        let object_stores = ObjectStores::new(self.object_store, self.wal_object_store);
        let fp_registry = Arc::new(FailPointRegistry::new());
        let system_clock = self
            ...
            .unwrap_or_else(|| Arc::new(DefaultSystemClock::new()));
        let rand = self.rand.unwrap_or_else(|| Arc::new(Default::default()));
        let mut stored = if self.shared_source_pins {
            crate::clone::create_clone_with_shared_source_pins_and_checkpoint(
                sources,
                clone_path,
                object_stores,
                fp_registry,
        ...
        let checkpoint = stored
            .write_permanent_current_checkpoint_if_absent_exact(checkpoint_id, checkpoint_name)
            .await
            .map_err(crate::Error::from)?;
        let checkpoint_manifest = if stored.id() == checkpoint.manifest_id {
        ...
        let manifest = crate::manifest::VersionedManifest::from_manifest(
            checkpoint.manifest_id,
            checkpoint_manifest,
        );
        Ok(CloneCheckpointResult {
            checkpoint,
            manifest,
        })
    }
