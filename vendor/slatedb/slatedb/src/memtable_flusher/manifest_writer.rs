    ...
    async fn apply_ready_batch(
            ...
            .await
        {
            Ok(checkpoint_results) => {
                // The batch is now durable in the manifest, so each SST is
                // protected as active and no longer needs staged protection.
...
mod tests {
    ...
    async fn should_emit_flushed_event_for_contiguous_uploads() {
        ...
        let staged_id = uploaded.segments[0].sst_handle.id;
        inner.staged_ssts.add([staged_id]);
        ...
        assert!(
            !inner.staged_ssts.snapshot().contains(&staged_id),
            "durable manifest publication must release staged protection"
        );
