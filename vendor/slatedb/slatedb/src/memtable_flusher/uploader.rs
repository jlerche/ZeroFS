...
impl UploadHandler {
    ...
    async fn upload_with_retry(&self, job: &UploadJob) -> Result<UploadedMemtable, SlateDBError> {
        ...
        // Resolve and protect the SST ids before any object becomes visible.
        // Protection remains until the manifest writer durably publishes the
        // SSTs, or is released below if this upload batch fails.
        let sst_ids = built
            .iter()
            .map(|sst| {
                job.segment_sst_ids
        ...
        // on the first fatal error and drops the remaining futures; on abort we
        ...
        let segments = match futures::future::try_join_all(
            built
                .iter()
                .zip(&sst_ids)
                .map(|(sst, sst_id)| self.upload_segment_sst(sst, *sst_id)),
        )
        .await
        {
...
mod tests {
    ...
    async fn should_write_sst_to_preallocated_id() {
        ...
        assert!(
            db.staged_ssts
                .snapshot()
                .contains(&SsTableId::Compacted(expected_id)),
            "an uploaded SST must remain protected until manifest publication"
        );
