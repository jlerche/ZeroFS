...
impl Admin {
    ...
    /// Create or recover a permanent, source-bound detached checkpoint with a
    /// caller-stable identity.
    ///
    /// ZeroFS uses this to elect one shared physical source pin for an exact
    /// immutable checkpoint across concurrent branch creators.
    pub async fn create_detached_checkpoint_with_id(
        &self,
        checkpoint_id: Uuid,
        options: &CheckpointOptions,
    ) -> Result<CheckpointCreateResult, crate::Error> {
        if options.source.is_none() || options.lifetime.is_some() {
            return Err(SlateDBError::CheckpointIdentityConflict(checkpoint_id).into());
        }
        let manifest_store = Arc::new(self.manifest_store());
        let mut stored_manifest =
            StoredManifest::load(manifest_store, self.system_clock.clone()).await?;
        ...
        stored_manifest
            ...
            .validate_wal_object_store_uri(configured_wal_uri.as_deref())?;
        let checkpoint = stored_manifest
            .write_checkpoint_if_absent_exact(checkpoint_id, options)
            .await?;
        Ok(CheckpointCreateResult {
            id: checkpoint.id,
            manifest_id: checkpoint.manifest_id,
    ...
    /// Create or recover a permanent checkpoint of the destination's current
    /// manifest with an exact caller-provided identity.
    ///
    /// This is intended for recovery after an initialized clone manifest is
    /// visible but the response that installed its stable root checkpoint was
    /// lost. A retry never retargets an existing ID or name.
    pub async fn create_current_checkpoint_with_id(
        &self,
        checkpoint_id: Uuid,
        checkpoint_name: &str,
    ) -> Result<CheckpointCreateResult, crate::Error> {
...
mod tests {
    ...
    #[tokio::test]
    async fn test_admin_stable_checkpoint_requires_and_recovers_exact_permanent_source() {
        let object_store: Arc<dyn ObjectStore> = Arc::new(InMemory::new());
        let path = Path::from("/tmp/test_admin_stable_checkpoint_exact_source");
        let db = crate::Db::open(path.clone(), Arc::clone(&object_store))
            .await
            .unwrap();
        db.put(b"key", b"value").await.unwrap();
        let source = db
            .create_checkpoint(
                crate::config::CheckpointScope::All,
                &CheckpointOptions::default(),
            )
            .await
            .unwrap();
        ...
        let options = CheckpointOptions {
            ...
            name: Some("stable-source-pin".to_string()),
        };

        let created = admin
            .create_detached_checkpoint_with_id(stable_id, &options)
            .await
            .unwrap();
        ...
        assert_eq!(recovered.manifest_id, created.manifest_id);
        let conflict = admin
            ...
            .await
            .unwrap_err();
        assert_eq!(conflict.kind(), ErrorKind::Data);
        let unsupported = admin
            .create_detached_checkpoint_with_id(uuid::Uuid::new_v4(), &CheckpointOptions::default())
            .await
            .unwrap_err();
        assert_eq!(unsupported.kind(), ErrorKind::Data);

        admin
            .create_detached_checkpoint(&CheckpointOptions {
                lifetime: Some(std::time::Duration::from_secs(60)),
                source: None,
                name: Some("ordinary-expiring".to_string()),
            })
            .await
            .expect("ordinary expiring checkpoint creation remains supported");
    }
