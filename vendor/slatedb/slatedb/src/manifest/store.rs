...
impl StoredManifest {
    ...
    pub(crate) async fn read_manifest(&self, id: u64) -> Result<Manifest, SlateDBError> {
        self.store.read_manifest(id).await
    }
    ...
    pub(crate) async fn write_checkpoint_if_absent_exact(
        &mut self,
        checkpoint_id: Uuid,
        options: &CheckpointOptions,
    ) -> Result<Checkpoint, SlateDBError> {
        if options.lifetime.is_some() {
            return Err(SlateDBError::CheckpointIdentityConflict(checkpoint_id));
        }
        let clock = Arc::clone(&self.clock);
        self.inner
            .maybe_apply_update(|sr| {
                let mut new_val = sr.object().clone();
                let expected = Self::new_checkpoint(
                    &new_val,
                    sr.id().into(),
                    clock.as_ref(),
                    checkpoint_id,
                ...
                if let Some(existing) = new_val.core.find_checkpoint(checkpoint_id) {
                    if existing.manifest_id != expected.manifest_id
                        || existing.name != expected.name
                        || existing.expire_time.is_some()
                    {
                        return Err(SlateDBError::CheckpointIdentityConflict(checkpoint_id));
                    }
                    return Ok(None);
                }
                new_val.core.checkpoints.push(expected);
                let mut dirty = sr.prepare_dirty()?;
                dirty.value = new_val;
                Ok(Some(dirty))
            })
        ...
        Ok(self
            .db_state()
            .find_checkpoint(checkpoint_id)
            .expect("exact checkpoint update applied but checkpoint not found")
            .clone())
    }

    pub(crate) async fn write_permanent_current_checkpoint_if_absent_exact(
        &mut self,
        checkpoint_id: Uuid,
        name: &str,
    ) -> Result<Checkpoint, SlateDBError> {
        ...
        self.inner
            .maybe_apply_update(|sr| {
                let mut new_val = sr.object().clone();
                if let Some(existing) = new_val.core.find_checkpoint(checkpoint_id) {
                    if existing.name.as_deref() != Some(name) || existing.expire_time.is_some() {
                        return Err(SlateDBError::CheckpointIdentityConflict(checkpoint_id));
                    }
                    return Ok(None);
                }
        ...
        Ok(self
            .db_state()
            .find_checkpoint(checkpoint_id)
            .expect("exact current checkpoint update applied but checkpoint not found")
            .clone())
    }
