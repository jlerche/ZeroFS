...
impl Manifest {
    ...
    pub(crate) fn cloned(
        ...
        // Inherited dependencies remain independently detachable. Sharing
        // them would require transitive lifetime accounting in the caller:
        // deleting one sibling could otherwise remove a pin still used by
        // another sibling. ZeroFS shares only the direct source checkpoint,
        // whose lifetime is tracked by its authoritative catalog.
        ...
        clone_external_dbs.push(ExternalDb {
            ...
            final_checkpoint_id: Some(if shared_source_pins {
                source_checkpoint_id
            } else {
                rand.rng().gen_uuid()
            }),
...
mod tests {
    ...
    fn test_cloned_breaks_chain_via_final_checkpoint() {
        ...
        let cloned = Manifest::cloned(&parent, "/tmp/parent".to_string(), parent_cp, rand, false);
    ...
    #[test]
    fn shared_pin_clone_borrows_only_direct_source_pin() {
        let rand = Arc::new(DbRand::default());
        let inherited_source = Uuid::new_v4();
        let inherited_final = Uuid::new_v4();
        let parent_checkpoint = Uuid::new_v4();
        let mut parent = Manifest::initial(ManifestCore::new());
        parent.external_dbs.push(ExternalDb {
            path: "/tmp/grandparent".to_string(),
            source_checkpoint_id: inherited_source,
            final_checkpoint_id: Some(inherited_final),
            sst_ids: vec![],
        });

        let cloned = Manifest::cloned(
            &parent,
        ...
        assert_eq!(cloned.external_dbs[0].source_checkpoint_id, inherited_final);
        assert_ne!(
            cloned.external_dbs[0].final_checkpoint_id,
            Some(inherited_final),
            "inherited dependencies need private pins so sibling detach is independent"
        );
        let direct = cloned
            .external_dbs
            .iter()
            .find(|external| external.path == "/tmp/parent")
            .unwrap();
        assert_eq!(direct.source_checkpoint_id, parent_checkpoint);
        assert_eq!(direct.final_checkpoint_id, Some(parent_checkpoint));
    }
