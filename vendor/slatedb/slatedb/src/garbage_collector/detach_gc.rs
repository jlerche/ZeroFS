...
impl DetachGcTask {
    ...
    async fn detach_from_parent(&self, external_db: &ExternalDb) -> Result<(), SlateDBError> {
        ...
        // ZeroFS shared pins are borrowed permanent source checkpoints. The
        // exhausted external entry may be removed, but one clone must never
        // delete a checkpoint still shared by sibling clones.
        if final_id == external_db.source_checkpoint_id {
            return Ok(());
        }
...
mod tests {
    ...
    #[tokio::test]
    async fn shared_pin_detach_removes_entry_without_deleting_parent_checkpoint() {
        let (object_store, parent_store, clone_store) = build_setup().await;
        let shared_id = Uuid::new_v4();
        attach_clone_to_parent(&parent_store, &clone_store, shared_id, vec![]).await;
        let mut clone = StoredManifest::load(clone_store.clone(), clock())
            .await
            .unwrap();
        clone
            .maybe_apply_update(|stored| {
                ...
                dirty.value.external_dbs[0].source_checkpoint_id = shared_id;
                Ok(Some(dirty))
            })
            .await
            .unwrap();

        make_task(clone_store.clone(), object_store)
            .collect(Utc::now())
            .await
            .unwrap();

        assert!(parent_has_checkpoint(&parent_store, shared_id).await);
        assert_eq!(clone_external_dbs_len(&clone_store).await, 0);
    }
