...
pub(crate) struct CompactedGcTask {
    ...
    /// with no co-located writer, which falls back to timestamp watermarks.
...
impl GcTask for CompactedGcTask {
    ...
    async fn collect(&self, utc_now: DateTime<Utc>) -> Result<(), SlateDBError> {
        ...
        //
        // Snapshot the writer's uploaded-but-not-yet-published SSTs first for
        // the same ordering reason: an SST captured here remains protected if
        // it retires into the manifest before the manifest read below.
...
mod tests {
    ...
    /// Build a staged-L0 scenario and run GC with the given staged set,
    /// returning the surviving compacted SST ids. The staged L0 is absent from
    /// the manifest and old enough to fall below every timestamp watermark.
    async fn run_staged_l0_gc(staged: Option<StagedSsts>) -> (SsTableId, Vec<SsTableId>) {
        ...
        table_store
            .write_sst(&staged_id, &build_test_sst(&format, 1).await)
            .await
            .unwrap();
        let published_handle = table_store
            .write_sst(&published_id, &build_test_sst(&format, 1).await)
            .await
            .unwrap();
        let mut dirty = stored_manifest.prepare_dirty().unwrap();
        Arc::make_mut(&mut dirty.value.core.tree)
        ...
        task.collect(DateTime::<Utc>::from_timestamp_millis(10_000).unwrap())
            .await
            .unwrap();
    ...
    #[tokio::test]
    async fn staged_l0_below_gc_watermarks_is_deleted_without_protection() {
        let (staged_id, remaining) = run_staged_l0_gc(None).await;
        assert!(!remaining.contains(&staged_id));
    }

    #[tokio::test]
    async fn staged_l0_below_gc_watermarks_is_protected_by_staged_set() {
