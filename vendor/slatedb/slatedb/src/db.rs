...
impl Db {
    ...
    /// Capture a snapshot pinned to the current remote durability watermark.
    /// Later flushes cannot change the rows visible through this snapshot.
    pub async fn durable_snapshot(&self) -> Result<Arc<DbSnapshot>, crate::Error> {
        self.inner.check_closed()?;
        let seq = self.inner.oracle.last_remote_persisted_seq();
        Ok(DbSnapshot::new(self.inner.clone(), Some(seq)))
    }
