...
- [x] Define catalog consistency and contention behavior.
  ...
  - [x] Route simultaneous branch-serving processes through one catalog authority; never open the same SlateDB catalog as multiple independent writers.
...
- [x] Provide a reviewed offline legacy-to-pool migration that preserves the exact volume key, reserves every imported epoch, rejects duplicate physical segment IDs across sources, and publishes authoritative completion; alternatively rewrite every colliding segment ID and `FrameLoc`.
...
- Projection selection: JSON and PostgreSQL implement the same reconstructible customer-facing projection. JSON defaults to `.zerofs/catalog-projection.json` for local development/tests; production selects PostgreSQL. PostgreSQL connections require certificate-verified TLS with the platform trust store by default and in production; only the explicit `tls = false` isolated-local-test setting selects plaintext, while externally managed callers may inject a `tokio_postgres::Client`.
...
### 2026-08-09: bounded customer projection queries

- The shared projection trait now exposes root-free point lookup plus stable UUID-ordered list pages with an optional branch/checkpoint filter. Both JSON and PostgreSQL enforce the same 256-record maximum, exclusive UUID cursor, ordering, and audit visibility for `deleted` and compacted `absent` records.
- JSON pages iterate the per-volume ordered map without loading storage authority; PostgreSQL issues the equivalent volume-scoped ordered query through its read connection. Neither path consults or returns SlateDB roots, manifests, leases, writer capabilities, private epochs, or GC state.
- Focused JSON coverage proves multi-page traversal, cross-kind ordering, kind filtering, and invalid bound/cursor rejection. The environment-gated PostgreSQL integration exercises the same two-page branch query alongside its existing no-storage-secret and customer-metadata preservation assertions. This query boundary is the customer-safe foundation for the still-pending branch CLI/RPC release surface.

### 2026-08-09: customer-safe branch read API

- The admin protocol now exposes `ListBranches` and exact-UUID `GetBranchInfo` over the selected customer projection. Responses contain stable volume/resource UUIDs, name, lifecycle state, historical parent/origin UUIDs, projection generation, timestamps, and customer metadata JSON; the schema contains no durable-root, manifest, lease, secret, private-epoch, or GC fields.
- The server receives a narrow `CustomerCatalog` capability rather than the authoritative lifecycle. It applies the shared branch filter and page bound, rejects invalid cursors/limits, reports an unavailable or unconfigured projection without falling back to SlateDB records, and still gates successful responses on current serving authority. Read-only `zerofs branch list` and `zerofs branch info` commands consume the same RPCs with explicit stable-UUID pagination.
- Conversion tests reject malformed metadata/timestamps, service tests prove checkpoint filtering, page traversal, bound enforcement, metadata delivery, and storage-secret absence, and a Unix-socket integration exercises the generated RPC client/server end to end. Mutating create/delete/checkpoint APIs remain pending and must commit to SlateDB first, then reconcile this projection.

### 2026-08-09: authoritative checkpoint publication primitive

- `CheckpointManager` now returns the exact durable SlateDB checkpoint UUID, manifest, database path, and normalized creation timestamp needed by catalog publication. Exact same-name creation retries return that physical identity, duplicate physical names fail closed, and public operations reject reserved internal names before storage I/O while listings hide internal checkpoints.
- `BranchLifecycle::publish_checkpoint` verifies the physical checkpoint's exact UUID, manifest, database path, public name, permanent lifetime, normalized creation time, and ready branch/database binding before atomically publishing the SlateDB catalog root. UUID and branch-scoped name retries must match every immutable input and do not advance generation; ambiguous catalog responses are reconciled and reauthenticated against storage. Internal, unnamed, differently named, expiring, or timestamp-mismatched physical checkpoints cannot be relabeled into the customer catalog.
- Exact physical deletion targets UUID plus expected name. A missing UUID is an idempotent success, a UUID/name mismatch conflicts, and replay can never delete a same-name replacement. Focused tests cover physical-create retry, reserved-name isolation, replacement-safe deletion, authenticated publication, conflicting name reuse, and generation-neutral retry. PostgreSQL and JSON remain identical root-free projections; RPC mutation wiring is the next release slice.

### 2026-08-09: authoritative checkpoint mutation RPC boundary

- A catalog-configured admin server receives only a branch-bound checkpoint capability. Creation first seals and durably checkpoints the active data plane, then publishes that exact UUID/path/manifest/name/permanence/timestamp descriptor through the SlateDB lifecycle before acknowledging success. A configured catalog without an active branch rejects before physical I/O; legacy non-catalog volumes retain their existing local checkpoint behavior.
- Delete requests carry stable checkpoint UUID plus historical name. The lifecycle derives the exact live or tombstoned revision from SlateDB, so a lost-response retry returns the old tombstone and cannot target a same-name replacement. The CLI resolves the UUID before mutation and exposes `--id` for an explicit exact retry.
- Logical deletion removes the customer name and named GC root but deliberately retains physical SlateDB checkpoint metadata until exact leases and incomplete operations drain. Successful create/delete commits reconcile the selected root-free projection on a best-effort basis; JSON and PostgreSQL still implement one identical projection contract and never become mutation authority.

### 2026-08-09: branch-bound customer checkpoint reads

- Catalog-configured checkpoint list/info RPCs now read only the selected root-free JSON/PostgreSQL projection. Their shared query contract adds identical parent-UUID and customer-state filters, so each bounded page contains only `ready` checkpoints owned by the active stable branch; configured reads never fall back to retained physical SlateDB checkpoint metadata.
- Checkpoint wire records expose stable volume/checkpoint/branch UUIDs, lifecycle state, projection generation, timestamps, and customer metadata JSON, with no root, manifest, lease, writer capability, private epoch, or GC state. List requests use the same 256-record UUID cursor bound as branch reads, and exact info accepts the stable checkpoint UUID while preserving bounded live-name lookup for CLI compatibility.
- Legacy volumes without a catalog retain their physical list/info behavior. Focused JSON and environment-gated PostgreSQL coverage exercise identical parent/state filtering, service coverage proves a physical-only checkpoint cannot leak through a configured server, and a Unix-socket round trip verifies the generated client preserves root-free customer metadata.

### 2026-08-09: authoritative branch mutation API
...
### 2026-08-09: reviewed offline legacy-to-pool migration

- `zerofs migrate-legacy-pool --confirm-offline` imports one non-replicated legacy database into its configured disjoint shared segment pool while every source and target-pool reader, writer, GC collector, and maintenance process remains stopped through command completion and the first successful root-admitting startup. It unwraps the legacy key and conditionally preserves the exact serialized wrapper, establishes authenticated pool authority, retains every source object as an offline rollback copy, and never changes a `Segid` or `FrameLoc`. Migration artifacts are not GC roots, so the CLI and operator contract make this full-pool quiescence explicit.
- The protocol publishes a fail-closed bootstrap, binds the source path to its persistent database-instance UUID and exact wrapped-key digest, durably fingerprints the initial inventory, preflights duplicate physical IDs, and permanently reserves every imported epoch. Pool-global create-only `Segid` claims prevent concurrent sources from sharing even byte-identical physical IDs; source/target SHA-256 verification and streamed claim reconciliation detect crash-time removals before authenticated bounded completion. Bootstrap/genesis/intent force completion even if legacy remnants are removed or the path is reused, while reservation validation point-checks each streamed segment with bounded memory.
- Password rotation targets the authoritative pool wrapper after migration. The retained legacy wrapper can consequently differ, while completion authentication under the unchanged DEK continues to prove compatibility. Focused tests cover byte-exact import and retry, multi-epoch reservation, concurrent cross-source physical-ID rejection, claimed-source removal across interruption, exact key preservation, database-incarnation reuse, and startup rejection without completion even when source remnants are absent.
- This storage slice exceeds the usual 800-line review guide because the create-only bootstrap, authenticated intent/global claims/completion, startup admission, exact-key handling, adversarial crash tests, CLI, and operator contract form one safety boundary. Splitting them would leave an intermediate commit that could create migration artifacts without the matching completion gate, or admit a pool without the protocol that proves its imported identities.

### 2026-08-09: supported-envelope root-capture probe

- Root-pin extraction no longer performs `Vec::contains` for every candidate (quadratic at scale). It collects once, then performs one canonical sort/deduplication. A focused test preserves typed ordering and duplicate behavior.
- The ignored release-mode `gc_root_capture_supported_envelope` probe constructs the exact declared ceiling of 4,096 branches plus 256 checkpoints per branch. On this qualification host it validated 1,052,672 roots in 377 ms, collected them in 169 ms, and computed the digest in 158 ms; the canonical encoded root set is 117,710,537 bytes.
- This evidence covers the root-capture CPU path only. The mark-generation-at-limit, external-I/O linearity, and concurrent foreground-latency rows remain unchecked until measured on representative object-store behavior; the in-memory probe is not used as evidence for them.

### 2026-08-09: GC external-work amplification characterization

- The ignored release-mode `gc_external_work_amplification_probe` drives the real SlateDB catalog and mark/report pipeline through an object-store boundary that counts ordinary requests, multipart initiations/parts/completions, successful transferred payload bytes, and streamed list results. It doubles one dimension at a time while holding the others fixed: 2 to 4 roots at 1,024 references per root, 20,000 to 40,000 same-shard references under one root, and 20,000 to 40,000 same-shard inventory objects. The reference and inventory pairs exceed the 8,192-record spill and 256 KiB artifact thresholds.
- Logical root/reference/inventory work doubles exactly. The original reference measurement grew multipart bytes from 902,447 to 2,329,081 (2.58x), exposing the binary-carry merge's additional external level and disproving a strict linear-amplification requirement. This characterization therefore did not close the old row; it motivated the derived production bound below. In-memory timings, provider pagination, supported-envelope mark duration, and concurrent foreground latency remain separately unqualified.

### 2026-08-09: derived GC merge-amplification bound
...
### 2026-08-09: representative-backend physical-open harness

- The ignored release-mode `gc_physical_checkpoint_open_representative_backend` harness provisions the exact supported physical shape under a unique object-store prefix: 4,096 independent SlateDB databases, each with one branch root and 256 checkpoint roots. It then drives the production physical checkpoint reader and mark pipeline across all 1,052,672 roots, requires completion below the existing 15-minute stalled-phase boundary, and reports fixture/mark duration plus object-store request and payload counts.
- The harness rejects local, memory, and plain HTTP targets, rejects credentials or options embedded in the URL, requires an exact acknowledgement of the remote cost and retained artifacts, and never cleans up automatically so failures remain auditable. A real 15-minute timeout emits explicit failure JSON with the retained prefix and partial request counters before failing. Operators use the exact library-target invocation to keep this expensive selection narrow and auditable. This tooling does not check the representative-backend row by itself; that row remains open until an approved production-class target completes the exact run and its JSON evidence and retained prefix are reviewed.

### 2026-08-09: offline shadow-GC report entry point

- `zerofs global-gc report --config <path> --confirm-offline [--run-id <UUID>]` gives operators a narrow production entry point for the existing terminal report lifecycle. It requires volume-wide quiescence before reading configuration, opens the configured authoritative SlateDB catalog and the same data, shared-pool, and WAL storage locations as serving, then invokes only capture, mark, and report. There is no command path to quarantine, revalidation, or deletion, and emitted JSON states that physical deletion capability is false.
- The command prints a generated or supplied run UUID before capture, so an interrupted invocation can be retried exactly. Completion JSON records the catalog generation, cutoff, root digest, root/reference/unique-segment totals, physical-inventory totals, and proposed candidate objects/bytes. PostgreSQL and JSON remain identical reconstructible customer projections and receive no GC state.
- This provides the invocation boundary needed to gather rollout evidence; it does not complete the shadow rollout row. That row remains unchecked until operators execute approved deployment runs and retain the backend identity, run UUIDs, configuration, metrics, ideal-model comparison, disagreement analysis, and representative completion/reclaim estimates required above.

### 2026-08-09: MinIO/PostgreSQL lifecycle stress qualification

- The ignored `cow_minio_postgres_lifecycle_stress` harness drives real MinIO/S3 object storage, the authoritative SlateDB catalog and physical roots, plaintext-or-TLS PostgreSQL, and the identical JSON projection without Redis. Its default retained-prefix workload covers 128 concurrent checkpoint clones with duplicate exact retries, a conflicting operation rebind, paginated projection parity and metadata, 16,384 references per root over 4,096 reachable segment objects, 512 unreachable candidates, writer fencing and head publication, exact terminal-report retries, projection reconciliation raced with duplicate deletes, catalog reopen, and a final rootless report.
- The local MinIO/PostgreSQL qualification completed 24 branch roots with 4,096 references each (98,304 physical references), 1,024 reachable objects, 128 candidates, and concurrency 8 in 78,115 ms. The rooted report enumerated 24 roots and 3,072 intermediate mark runs, deduplicated to 1,024 segments, and classified all 1,152 inventory objects exactly; after deletion and reopen, all 1,152 were candidates. It issued 26,756 object GETs, 1,214 lists, and 9,464 puts under retained prefix `local-run/zerofs-cow-stress/aaa66206-27b2-470c-9dfa-5c47bd09bbeb`.
- Iterative MinIO runs exposed three production defects now covered by focused regressions or the retained systems test: simultaneous exact clones could observe the winning clone's uninitialized manifest, S3 LIST/GET timestamp precision disagreed during unchanged candidate identity checks, and projection snapshots could reject the valid overlap of a published create with its branch's draining delete. Clone reconciliation is now bounded to 30 seconds and fails closed on timeout or ownership conflict; GC compares S3 metadata at common whole-second precision while retaining size and streamed digest revalidation; catalog validation accepts `Deleting` only when an exact draining delete operation exists.
- This is local adversarial integration evidence, not production rollout evidence. It does not check the representative-backend physical-open or shadow-rollout rows, and retained prefixes require explicit operator cleanup.

### 2026-08-09: offline genesis branch bootstrap

- `zerofs branch bootstrap` supplies the missing production boundary for the first catalog branch. It requires `--confirm-offline` before reading configuration, the explicitly enabled default-off `catalog.authority.features.create` control, rejects an active `[catalog.mount]`, resolves one exact permanent physical checkpoint from the configured legacy/base database, and runs the same reserved/root-created/published clone protocol used by descendants. Generated or supplied branch and operation UUIDs are printed for exact retry; the confirmation is an operator attestation rather than a concurrency lock.
- The only new operation shape is `parent_id = None`: SlateDB admits it only when no branch or branch-create operation exists, records lineage depth zero and no customer origin checkpoint, and permanently rejects a second genesis. Ordinary descendant reservations still require an authoritative source branch/checkpoint and retain their existing blocker/index transaction. JSON and PostgreSQL receive the same parentless root-free customer record; the external physical checkpoint and storage proof remain SlateDB/object-store authority only.
- Focused lifecycle coverage proves exact retry, second-genesis rejection, authenticated destination publication, and descendant database reopen. Phase-specific recovery tests prove a Reserved retry fails closed after source-name reuse, while a RootCreated retry publishes after the physical source checkpoint is deleted. Catalog tests verify the source root remains GC-pinned without a customer checkpoint hold or parent blocker, and runtime coverage verifies resume catches the JSON/PostgreSQL-shaped customer projection up. CLI coverage proves the offline acknowledgement is checked before configuration I/O. The next systems slice uses this boundary to run and fork real PostgreSQL data directories on MinIO-backed ZeroFS.

### 2026-08-09: PostgreSQL-on-ZeroFS branch stress

- `zerofs/scripts/stress-postgres-branches.sh` provisions disposable MinIO and PostgreSQL containers without Redis, initializes a real PostgreSQL data directory through strict 9P/FUSE ZeroFS, bootstraps the parent catalog branch, creates a fan-out checkpoint and multiple COW branches, and runs concurrent divergent `pgbench` writes while every branch server remains live. It verifies inherited baseline data, UUID-specific child isolation, parent isolation, `pg_amcheck`, hard PostgreSQL recovery, hard ZeroFS worker recovery, and removal of generated containers, mounts, processes, and workspace on success. The customer projection is a separate plaintext local PostgreSQL container; SlateDB remains lifecycle, root, lease, and GC authority.
- The first physical run exposed a required S3 setting that metadata-only tests did not exercise: PostgreSQL crossed the segment multipart threshold, and immutable publication failed until AWS/MinIO `copy_if_not_exists = "multipart"` was configured. MinIO's native ordinary conditional PUT and multipart-completion preconditions need no Redis. Generated configuration and operator documentation now call out both independent requirements.
- Simultaneous branch workers now use one private same-host, single-service-account catalog writer authority. Before opening SlateDB, the owner claims a canonical per-uid local lock keyed by stable volume UUID (independent of the configured endpoint) and binds a mode-`0600` Unix socket inside a real, same-uid directory with no group/other permissions. A second supported local owner therefore fails before it can fence SlateDB, and another local user cannot replace or impersonate the socket. Workers configured with `connect_unix_socket` prepare, renew, recover, and publish exact writer grants and publish/delete branch-local checkpoints through that service while suppressing their own catalog open. The capability service is never registered on customer/admin TCP listeners, while workers open the configured PostgreSQL/JSON projection for root-free customer reads; customer branch/checkpoint records continue to contain no roots, manifests, lease tokens, or renewal secrets. PostgreSQL and JSON remain identical reconstructible customer projections rather than catalog authorities. Multi-host or mixed-uid authority requires a future authenticated/distributed transport and remains outside this Unix-socket topology.
- The fuller local qualification kept three branch servers and their PostgreSQL instances live together at pgbench scale 2, with twelve concurrent clients across the branches for ten seconds of divergent transaction load. It recovered one killed PostgreSQL container and one killed ZeroFS worker without interrupting the shared authority or sibling branches, passed every database through `pg_amcheck`, verified exact child UUID markers, and proved the live parent never observed the child-only table. A subsequent two-branch run hard-killed and restarted the sole catalog owner, waited through a complete worker renewal interval, and proved both existing worker channels reconnected and renewed before exercising the data-plane crash cases. These local runs are adversarial integration evidence, not representative-backend or multi-host rollout evidence.

### 2026-08-09: main-to-production data-plane performance comparison

- `zerofs/scripts/compare-main-performance.sh` builds one exact `main` revision and the exact candidate index tree in detached worktrees with each checkout's full-LTO release profile; unstaged and untracked files therefore cannot affect either binary. The shared benchmark driver is built from that same exact candidate tree, and all three cache keys include a digest of the compiler plus the complete normalized process environment, covering Cargo profile and target-specific overrides. Summaries record the candidate base commit/tree, build-environment digest, SHA-256 of both ZeroFS binaries, benchmark driver, and live harness, Rust/Cargo versions, and the pulled container tags plus immutable IDs actually used to launch each container. Both binaries use the same local MinIO, 9P, and strict FUSE path, with a fresh object prefix and cache for every sample. Trials alternate `main`/candidate order, perform an untimed warm-up, fail on any benchmark operation error, initialize a real PostgreSQL 17 data directory for durable `pgbench`, retain raw JSON, and emit median/range plus paired-trial mean/median deltas. Resource intent is registered before synchronous launches, signal-triggered exit is deferred across asynchronous PID registration and checked by boundary self-tests, every container carries a random run-ownership label, and all cleanup probes and removals are independently bounded. The watchdog grants cleanup more than its calculated serial worst case; failed unmount/process/container/worktree cleanup fails the run, and the evidence directory is never recursively deleted. This comparison covers existing filesystem data-plane behavior with the catalog disabled; branch lifecycle throughput, representative cloud latency, and multi-host authority transport require separate measurements.
- The first retained ten-trial microbenchmark run `45a26782eaa54e35a6acca5f15f9bb6b` compared `main` `26769980a5fd96fc99b30932f725631d733c1750` with candidate commit `1c3ba326629c0ed51aefc56560080d08c0f83e52`. Candidate median throughput changed by -0.86% for empty-file creation, +0.11% for metadata reads, -1.35% for random reads, -0.25% for sequential writes, and +1.66% for single-file append. Paired means ranged from -1.07% to +1.37%; every two-sided 95% paired confidence interval included zero, with the most negative lower bound at -2.74%.
- Adding durable PostgreSQL initially exposed a real screening failure: five paired 30-second runs measured a -8.66% candidate median and -6.26% paired mean (`95% CI [-12.32%, -0.19%]`). Repeating with `ignore_fsync = true` reduced the median difference to -2.21%, localizing most of the regression to the small-segment durable-publication path. Immutable create-only publication remains required for shared branch-pool correctness. The avoidable cost was the new rotating store wrapper cloning an `Arc` through `ArcSwap` on every segment operation even though writer-epoch rotation is rare. Its ordinary path now borrows the initial store without atomic reference-count traffic, while epoch rotation retains the existing atomic acquire/release handoff and background tasks request an owned snapshot.
- The optimized exact staged candidate tree `3c03ee04054d9b22fc7eb6b707791342dbe1c56a` then passed ten paired 30-second trials (`4e7314a4ce9b4f7f86bde36655aaba1e`). Paired medians were +1.05% empty files, +0.67% metadata, +1.33% random reads, +0.16% sequential writes, -0.55% append, and +2.21% durable PostgreSQL; paired means ranged from -0.45% to +1.24%. Because short durable samples remained noisy, five paired 120-second trials (`e0256371d2df4bb5b05d1ae514607f0e`) followed: PostgreSQL's candidate median was 4.160930 TPS versus main's 4.043356 TPS, with a +6.05% paired median and +5.05% paired mean (`95% CI [-0.01%, +10.12%]`). No local regression remains under the 5% screen; retained local evidence does not substitute for the still-open representative-backend qualification.

### 2026-08-16: focused branch-creation throughput benchmark

- The ignored release-mode `cow_branch_create_throughput` benchmark times only the production reserve, physical COW clone, root authentication, authoritative publication, and optional customer-projection acknowledgement after fixture setup and warm-up. It accepts 1–4,095 timed branches, 1–256-way concurrency, configurable source references/segments, and authority-only, PostgreSQL, or JSON acknowledgement modes. The PostgreSQL mode uses the same server-facing Rust lifecycle and synchronous projection boundary as production, but deliberately does not claim to measure network RPC authentication/authorization or transport overhead. Completion JSON reports branches/second, p50/p95/p99/min/max operation latency, exact retained MinIO prefix, and timed GET/PUT/LIST requests per branch; guards require an explicit retained-prefix acknowledgement and reject local/non-S3 or credential-bearing URLs.
- The local qualification used release code, MinIO and PostgreSQL 17 on Docker `overlay2` local anonymous disk volumes, eight untimed warm-ups, and 256 one-reference branches per authority sample. Concurrency 1/8/32/64 delivered 3.293/3.298/3.298/3.298 branches per second. Corresponding p50 latency was 303 ms, 2.425 s, 9.702 s, and 19.409 s; p95 was 305 ms, 2.628 s, 9.726 s, and 19.711 s. Timed object-store requests per branch rose from 75.09 to 94.61 under contention. Exact retained qualification prefixes are printed under `runs/e1abf32517b64dd1bfb2e0293896fee4` in bucket `zerofs-branch-bench`.
- Source cardinality and customer projection were not the ceiling. At concurrency 32, 128 branches from a 4,096-reference/1,024-segment source produced 3.298 branches/s with 9.702 s p50. The one-reference PostgreSQL and JSON modes produced 3.293 and 3.295 branches/s with 9.714 s and 9.707 s p50. Every concurrency therefore saturated at the same ~3.30 branches/s while latency grew almost exactly with queue depth.
- The bottleneck-removal follow-up preserves the durable pre-clone reservation but combines authenticated root recording and ready publication into one atomic batch. Independent catalog mutations are admitted as separately atomic buffered batches under serialized validation, then share concurrent SlateDB durability fences; readers use SlateDB's remote-durability filter and generation-stable retries instead of draining every pending group. Caller cancellation cannot cancel the detached `apply` durability task, branch-create readers never expose an unconfirmed record, and any ambiguous `apply` admission/barrier failure permanently poisons the open catalog until restart. Schema v21 also migrates and snapshot-audits an atomic live-branch counter so capacity admission no longer performs an O(n) branch scan.
- Four final disk-backed MinIO trials of 256 one-reference branches at concurrency 32 delivered 60.338–63.373 branches/s, with a 61.818 branches/s median: 18.74x the 3.298 branches/s baseline. Median operation latency was 404–408 ms rather than 9.702 s. A 64-branch concurrency-1 endpoint delivered 4.946 branches/s with 202 ms p50, while concurrency 64 delivered 47.848 branches/s with 1.008 s p50; this local host therefore saturates around concurrency 32 rather than improving with a deeper queue. Qualification prefixes were `runs/final-fastpath-c32`, `runs/final-fastpath-c32-t2`, `runs/final-fastpath-c32-t3`, `runs/final-fastpath-c32-t4`, `runs/final-fastpath-c1`, and `runs/final-fastpath-c64` in bucket `zerofs-branch-breakthrough`; exact result JSON and a manifest are retained outside the repository under `~/.local/state/codex-desktop/tmp/zerofs-branch-breakthrough-20260816`. MinIO used Docker `overlay2` disk storage, not tmpfs.
- The historical baseline bottleneck matched the former authoritative implementation: `SlateDbCatalog::apply` held one catalog-wide mutex through an `await_durable` SlateDB write, and a successful branch needed three serialized catalog transitions (reserve, root-created, publish). The former ~303 ms concurrency-one latency was consistent with three roughly 100 ms durability waits, while higher concurrency could not overlap them. The first follow-up removed that boundary by reducing the normal lifecycle to two durable phases and grouping durability for independent atomic mutations; it is baseline diagnosis rather than a description of the current hot path.
- The production-path follow-up now gives every new descendant reservation an authoritative shared-source mode marker. One deterministic hidden permanent physical checkpoint retains the immutable source while the customer-named checkpoint and every ancestor remain logically deletable. A fresh clone installs its permanent operation root checkpoint in the initial conditional manifest write, borrows only the direct catalog-managed pin, retains private inherited pins, and publishes one operation-owned result. The clone builder reuses the already-authenticated immutable source manifest, but ZeroFS still verifies the completed root, retained pins, and every reachable SST before catalog publication. Physical root checkpoint UUIDs use a separate deterministic namespace from catalog operation/resource UUIDs, preserving deep-lineage checkpoint promotion and global catalog ID uniqueness. Pre-v22 operations remain on the private-pin recovery path.
- Projection waiters now read only the durable catalog generation; one elected reconciler captures the full durable snapshot and advances PostgreSQL/JSON through at least every caller's generation. A regression test proves 32 concurrent waiters perform one full projection snapshot. PostgreSQL keeps normal synchronous commit, applies changed customer-visible branch/checkpoint/tombstone rows in one set-based transaction, preserves customer metadata, and forces a full reconciliation on the next required generation advance whenever the process-local cache does not match PostgreSQL's locked observed generation. SlateDB remains authoritative for internal lifecycle, roots, pins, leases, GC, and recovery state; PostgreSQL and JSON remain equivalent root-free customer projections rather than authorities.
- With disk-backed local MinIO and disk-backed PostgreSQL 17, no Redis, plaintext PostgreSQL only for the local connection, and normal synchronous PostgreSQL commit, the exact production-path benchmark reached 335.955 branches/s for 512 one-reference branches at concurrency 160 (`05590f76-0dbe-445c-99c7-bdb9dc486b0b`), 5.43x the committed 61.818 branches/s median. The exact staged-tree 1,024-branch run at concurrency 128 sustained 265.215 branches/s (`7d720138-ea01-42fd-8e70-70313ac1ad6d`), 4.29x, with 409.5 ms p50 and 612.7 ms p95; a preceding production-path run reached 297.043 branches/s, exposing meaningful local-run variance rather than a guaranteed sustained rate. An earlier concurrency-192 run reached only 278.515 branches/s, so deepening the queue did not reliably improve this local MinIO setup. Reusing authenticated immutable source state reduced timed object-store amplification from about 18.04 to 11.04 requests per branch while retaining final physical verification. These are local production-code-path results, not representative-cloud latency or authenticated RPC results.
- A follow-up production-path optimization removes an unused shared-destination pre-read and coalesces only overlapping physical proofs of the exact permanent hidden source pin. Completed and failed proofs are evicted immediately, caller cancellation cannot cancel the detached proof, and every later wave touches storage again; destination identity and reachable-SST checks remain per branch. On the same disk-backed services, synchronous-PostgreSQL concurrency-160 runs measured 419.749 branches/s over 1,024 branches at 6.187 requests/branch (`6e95f539-c7fe-4e76-ba82-3a5cb74eb1c9`) and 274.127 branches/s over the maximum 4,087 timed branches plus eight warm-ups at 6.293 requests/branch (`53841ea8-0a57-4486-9c1b-fccb408f0d4e`). Authority-only 4,087-branch repeats measured 266.118, 271.499, and 257.692 branches/s at 6.35 requests/branch, confirming that PostgreSQL is not the sustained ceiling. Across two source-identical sustained authority runs, MinIO consumed roughly 1.3-1.5 CPU cores while NVMe utilization remained below 23%; the remaining local ceiling therefore points to small-object/conditional-write processing and metadata serialization in the single-node MinIO path rather than PostgreSQL or raw disk bandwidth. The immutable destination manifest and operation-result object still impose about two PUTs per branch; removing the result object would require a separately reviewed recovery-format change rather than a benchmark-only shortcut.
- A real AWS S3 qualification used the exact committed release source, a dedicated private `us-east-1` bucket in the explicitly verified target account, and the same production lifecycle with normal synchronous PostgreSQL commit. The 64-branch concurrency-32 smoke test reached 18.273 branches/s at 6.453 requests/branch (`7e910626-b701-43e7-a0cb-3ac27f9ea0cf`). Over 1,024 branches, concurrency 160 and 256 reached 64.860 and 84.155 branches/s at 6.112 and 6.079 requests/branch (`33f6fc46-423b-428f-8a3a-95378af2c482`, `2ae806a1-1408-48f8-a382-8996a1d98498`). The maximum 4,087-branch concurrency-256 run sustained 104.472 branches/s with synchronous PostgreSQL, 2.174 s p50, 3.219 s p95, and 6.069 requests/branch (`1381b1f9-10bb-48fb-955c-89cb406d0a85`); the matching authority-only run reached 100.811 branches/s and 6.066 requests/branch (`8ba699a0-8c6d-42f2-b111-0c36d4a8cee7`). PostgreSQL is therefore not the measured S3 ceiling. Unlike local MinIO, S3 held the longer-run request rate, while multi-request WAN latency kept individual creates around two to four seconds; reducing serial durability/publication round trips is the next representative-backend optimization target.
