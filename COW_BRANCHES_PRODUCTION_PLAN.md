...
- The measured shape matches the authoritative implementation: `SlateDbCatalog::apply` holds one catalog-wide mutex through an `await_durable` SlateDB write, and a successful branch needs three serialized catalog transitions (reserve, root-created, publish). The ~303 ms concurrency-one latency is consistent with three roughly 100 ms durability waits; higher concurrency cannot overlap them. Hundreds of branches per second are not achievable with this boundary. The next optimization slice must group durability for independent catalog mutations while preserving atomic per-mutation batches, monotonic generation/revision checks, acknowledged-durability semantics, ambiguous-response reconciliation, and fail-closed GC roots.
