...
- [x] Define catalog consistency and contention behavior.
  ...
  - [ ] Route simultaneous branch-serving processes through one catalog authority; never open the same SlateDB catalog as multiple independent writers.
...
- A simultaneous two-branch-server probe exposed a remaining production architecture gap. Every serving process currently opens the same SlateDB catalog as an independent writer; the newest open fences older catalog clients, their lease renewal fails closed, and serving stops. The passing harness therefore rotates one catalog authority between branch servers while retaining every branch root and PostgreSQL data directory. This is not evidence for simultaneous branch serving: that requires a central catalog-authority RPC (or equivalent single-writer service) and remains explicitly unchecked above.
- The fuller local qualification completed three branches at pgbench scale 2 with four clients per branch and ten seconds of divergent transaction load. It recovered one killed PostgreSQL container and one killed ZeroFS server, passed every database through `pg_amcheck`, verified exact child UUID markers, and proved the parent never observed the child-only table. A prior reduced two-branch run exercised the same path. These local runs are adversarial integration evidence, not representative-backend or multi-server rollout evidence.
