...
- The ignored release-mode `cow_branch_create_throughput` benchmark times only the production reserve, physical COW clone, root authentication, and authoritative publication protocol after fixture setup and warm-up. It accepts 1–4,095 timed branches, 1–64-way concurrency, configurable source references/segments, and authority-only, PostgreSQL, or JSON acknowledgement modes. Projection modes include the same synchronous full-snapshot reconciliation attempted after the authoritative mutation by the server boundary. Completion JSON reports branches/second, p50/p95/p99/min/max operation latency, exact retained MinIO prefix, and timed GET/PUT/LIST requests per branch; guards require an explicit retained-prefix acknowledgement and reject local/non-S3 or credential-bearing URLs.
...
- The baseline bottleneck matched the former authoritative implementation: `SlateDbCatalog::apply` held one catalog-wide mutex through an `await_durable` SlateDB write, and a successful branch needed three serialized catalog transitions (reserve, root-created, publish). The former ~303 ms concurrency-one latency was consistent with three roughly 100 ms durability waits, while higher concurrency could not overlap them. The follow-up above removes that boundary by reducing the normal lifecycle to two durable phases and grouping durability for independent atomic mutations. Concurrency 64 then coincides with lower throughput and increased aggregate object-store request amplification; separating physical clone, catalog, and shared object-store costs requires follow-up component profiling.
