...
- The ignored release-mode `gc_external_work_amplification_probe` drives the real SlateDB catalog and mark/report pipeline through an object-store boundary that counts ordinary requests, multipart initiations/parts/completions, successful transferred payload bytes, and streamed list results. It doubles one dimension at a time while holding the others fixed: 2 to 4 roots at 1,024 references per root, 20,000 to 40,000 same-shard references under one root, and 2,048 to 4,096 inventory objects. Both reference fixtures exceed the 256 KiB artifact threshold and therefore exercise production multipart writes.
- Logical root/reference/inventory work doubles exactly. On the qualification host, root doubling changed mark GETs from 1,066 to 1,108 and GET bytes from 201,986 to 320,308; reference doubling changed mark GETs from 803 to 1,086, GET bytes from 2,358,657 to 5,213,644, and multipart work from 3 initiations/6 parts/902,447 bytes to 5 initiations/13 parts/2,329,081 bytes; inventory doubling changed report GETs from 4,608 to 6,656, GET bytes from 571,680 to 1,007,904, and streamed objects/bytes exactly from 2,048/18,432 to 4,096/36,864.
- The 2.58x multipart-byte increase when references double exposes the binary-carry merge's additional external merge level; it is not linear amplification. The row remains open pending either a derived supported-envelope bound that explicitly accounts for merge levels or a linear external-sort redesign. Root and inventory samples are also below the supported root envelope and the 8,192-record inventory spill threshold, respectively. In-memory timings, provider pagination, supported-envelope mark duration, and concurrent foreground latency remain unqualified.
