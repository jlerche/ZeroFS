...
- Catalog configuration currently fails closed when `[replication]` is present. HA catalog ownership remains open until catalog open/close and projection reconciliation follow elected active ownership and takeover, preventing two nodes from opening one authoritative SlateDB catalog as independent writers.
