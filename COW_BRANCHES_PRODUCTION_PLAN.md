...
- Non-expiring exclusion: a live guard blocks permanent epoch exposure, branch deletion, checkpoint publication, clone-source capture, and new branch/checkpoint leases. There is intentionally no timeout, revocation, or generic release mutation; the future bounded deletion-progress transition must durably account for all outcomes before guard retirement can be added.
- Storage split and enablement: guards remain SlateDB-only authority and never enter identical PostgreSQL/JSON customer projections. Publisher drain/seal proof, exact local liveness enumeration, deletion progress, and guard retirement are not yet implemented, so private physical deletion remains disabled.
