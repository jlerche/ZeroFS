...
- Catalog schema v19 maintains one SlateDB-only branch-to-writer index in the same atomic batch as lease acquisition/release. A branch admits exactly one writer lease; exact acquisition retry remains generation-neutral, a competing writer fails before publication, branch deletion point-reads the index, and complete snapshots audit it against authoritative lease records. PostgreSQL and JSON remain identical and receive neither leases nor the private index.
- A generic expiry mutation may retire bounded read leases after the five-minute deadline plus 30-second skew, but it cannot discard a writer record. Once its time authority expires, that record is a crash-recovery blocker until a fenced head-publication path performs exact release. Migration rebuilds the index, accepts zero or one writer per branch, rejects multiple writers without advancing the prior schema marker, and repeats the same failure after partial migration work.
